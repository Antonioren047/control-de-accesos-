...
final class CronService
{
    ...
    private function cleanup():int{$dir=$this->root.'/storage/temp';if(!is_dir($dir))return 0;$count=0;foreach(new \FilesystemIterator($dir)as$file){if($file->getFilename()==='.gitkeep')continue;if($file->isFile()&&$file->getMTime()<time()-86400&&unlink($file->getPathname()))$count++;}return$count;}
}
