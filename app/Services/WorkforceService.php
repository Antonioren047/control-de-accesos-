...
final class WorkforceService
{
    ...
    public function update(array $actor,string $entity,int $id,array $input):array
    {
        if($id<=0)throw new HttpException('El registro indicado no es válido.',422);
        try{return match($entity){'guard'=>$this->updateGuard($actor,$id,$input),'shift'=>$this->updateShift($actor,$id,$input),'assignment'=>$this->updateAssignment($actor,$id,$input),default=>throw new HttpException('La entidad indicada no existe.',404)};}
        catch(PDOException $e){if((string)$e->getCode()==='23000')throw new HttpException('El número de empleado, correo o registro indicado ya existe.',409);throw $e;}
    }

    public function action(array $actor,string $action,array $input):array
    {
        $guardId=(int)($input['guard_id']??0);$shiftId=(int)($input['shift_id']??0);$assignmentId=(int)($input['assignment_id']??0);
        return match($action){
            ...
            'set_guard_active'=>$this->setGuardActive($actor,$guardId,(bool)($input['is_active']??false)),
            'set_shift_active'=>$this->setShiftActive($actor,$shiftId,(bool)($input['is_active']??false)),
    ...
    private function updateGuard(array $actor,int $id,array $input):array
    {
        $this->authorization->require($actor,'guards.manage');$this->requireManagedGuard($actor,$id);$required=['full_name','employee_number','phone','address_line','hire_date','emergency_contact_name','emergency_contact_phone'];$errors=Validator::required($input,$required);if($errors)throw new HttpException('Completa los datos obligatorios del vigilante.',422,$errors);
        $employee=strtoupper(trim((string)$input['employee_number']));if(!preg_match('/^[A-Z0-9_-]{2,40}$/',$employee))throw new HttpException('El número de empleado no es válido.',422);$email=strtolower(trim((string)($input['email']??'')));if($email!==''&&!Validator::email($email))throw new HttpException('El correo no es válido.',422);$curp=strtoupper(trim((string)($input['curp']??'')));if($curp!==''&&!preg_match('/^[A-Z0-9]{18}$/',$curp))throw new HttpException('La CURP debe contener 18 caracteres.',422);$photo=!empty($input['photo_data'])?$this->assets->savePhoto((string)$input['photo_data']):null;
        $this->repository->updateGuard($id,['full_name'=>trim((string)$input['full_name']),'employee_number'=>$employee,'photo_path'=>$photo,'phone'=>trim((string)$input['phone']),'email'=>$email,'curp'=>$curp,'address_line'=>trim((string)$input['address_line']),'hire_date'=>(string)$input['hire_date'],'emergency_contact_name'=>trim((string)$input['emergency_contact_name']),'emergency_contact_phone'=>trim((string)$input['emergency_contact_phone'])]);$this->audit($actor,'workforce.guard_updated',['guard_id'=>$id]);return['id'=>$id];
    }
    private function updateShift(array $actor,int $id,array $input):array
    {
        $this->authorization->require($actor,'shifts.manage');$this->requireAdmin($actor);if(!$this->repository->shiftInCompany($id,(int)$actor['surveillance_company_id']))throw new HttpException('El turno no existe.',404);$errors=Validator::required($input,['name','start_time','end_time','days','location_ids']);if($errors)throw new HttpException('Completa los datos obligatorios del turno.',422,$errors);$days=$this->days($input['days']);$locations=$this->ids($input['location_ids']);if(!$locations)throw new HttpException('Selecciona al menos un lugar.',422);foreach($locations as $location)if(!$this->scope->location($actor,$location))throw new HttpException('Uno de los lugares está fuera de tu alcance.',403);foreach(['start_time','end_time']as$f)if(!preg_match('/^(?:[01]\d|2[0-3]):[0-5]\d$/',(string)$input[$f]))throw new HttpException('La hora del turno no es válida.',422);$d=['name'=>trim((string)$input['name']),'start_time'=>$input['start_time'],'end_time'=>$input['end_time'],'crosses_midnight'=>$input['end_time']<=$input['start_time'],'tolerance_minutes'=>$this->minutes($input['tolerance_minutes']??10),'early_tolerance'=>$this->minutes($input['early_departure_tolerance_minutes']??0),'overtime_minutes'=>$this->minutes($input['overtime_after_minutes']??15),'days'=>$days,'location_ids'=>$locations];$this->pdo->beginTransaction();try{$this->repository->updateShift($id,$d,(int)$actor['id']);$this->pdo->commit();}catch(\Throwable$e){if($this->pdo->inTransaction())$this->pdo->rollBack();throw$e;}$this->audit($actor,'workforce.shift_updated',['shift_id'=>$id]);return['id'=>$id];
    }
    private function updateAssignment(array $actor,int $id,array $input):array
    {
        $this->authorization->require($actor,'assignments.manage');$this->requireAdmin($actor);if(!$this->repository->assignmentInScope($id,$actor))throw new HttpException('La asignación no existe o está fuera de tu alcance.',404);$errors=Validator::required($input,['guard_user_id','client_id','location_id','access_point_id','shift_id','start_date','days']);if($errors)throw new HttpException('Completa los datos obligatorios de la asignación.',422,$errors);$d=['guard_user_id'=>(int)$input['guard_user_id'],'client_id'=>(int)$input['client_id'],'location_id'=>(int)$input['location_id'],'access_point_id'=>(int)$input['access_point_id'],'shift_id'=>(int)$input['shift_id'],'start_date'=>(string)$input['start_date'],'end_date'=>trim((string)($input['end_date']??'')),'days'=>$this->days($input['days']),'assignment_type'=>(string)($input['assignment_type']??'regular'),'rotation_pattern'=>trim((string)($input['rotation_pattern']??''))];if(!in_array($d['assignment_type'],['regular','rotation','temporary'],true))throw new HttpException('El tipo de asignación no es válido.',422);if(!$this->scope->location($actor,$d['location_id'])||!$this->scope->accessPoint($actor,$d['access_point_id']))throw new HttpException('El alcance de la asignación no está autorizado.',403);if(!$this->repository->guardInCompany($d['guard_user_id'],(int)$actor['surveillance_company_id']))throw new HttpException('El vigilante no pertenece a la empresa o está inactivo.',422);if(!$this->repository->assignmentRelations($d['client_id'],$d['location_id'],$d['access_point_id'],$d['shift_id']))throw new HttpException('Cliente, lugar, punto y turno no corresponden entre sí.',422);if($d['end_date']!==''&&$d['end_date']<$d['start_date'])throw new HttpException('La fecha final no puede ser anterior a la inicial.',422);if($this->repository->overlappingAssignments($d['guard_user_id'],$d['start_date'],$d['end_date']?:null,$d['days'],$id))throw new HttpException('El vigilante ya tiene una asignación activa que se traslapa.',409);$this->repository->updateAssignment($id,$d,(int)$actor['id']);$this->audit($actor,'workforce.assignment_updated',['assignment_id'=>$id]);return['id'=>$id];
    }
    private function setGuardActive(array $actor,int $id,bool $active):array{$this->authorization->require($actor,'guards.manage');$this->requireManagedGuard($actor,$id);$this->repository->setGuardActive($id,$active,(int)$actor['id']);$this->audit($actor,'workforce.guard_status_changed',['guard_id'=>$id,'is_active'=>$active]);return[];}
    private function setShiftActive(array $actor,int $id,bool $active):array{$this->authorization->require($actor,'shifts.manage');$this->requireAdmin($actor);if(!$this->repository->shiftInCompany($id,(int)$actor['surveillance_company_id']))throw new HttpException('El turno no existe.',404);$this->repository->setShiftActive($id,$active,(int)$actor['id']);$this->audit($actor,'workforce.shift_status_changed',['shift_id'=>$id,'is_active'=>$active]);return[];}
    ...
    private function requireManagedGuard(array $actor,int $id):void{$this->requireAdmin($actor);if($id<=0||!$this->repository->guardExistsInCompany($id,(int)$actor['surveillance_company_id']))throw new HttpException('El vigilante no existe.',404);}
