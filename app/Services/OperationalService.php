...
final class OperationalService
{
    ...
    public function start(array $in):array
    {
        ...
        $credential=$this->repository->credential(hash('sha256',trim((string)$in['qr_token'])));if(!$credential||!(bool)$credential['is_active']||$credential['guard_status']!=='active')throw new HttpException('La credencial o el PIN no son vÃ¡lidos.',401);
    ...
    {
        $row=$this->current();$token=trim((string)($in['qr_token']??''));$credential=$token===''?null:$this->repository->credential(hash('sha256',$token));if(!$credential||(int)$credential['guard_user_id']!==(int)$row['guard_user_id'])throw new HttpException('Escanea la credencial QR vigente del vigilante para cerrar.',401);
