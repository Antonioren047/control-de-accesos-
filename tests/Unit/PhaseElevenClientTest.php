...
final class PhaseElevenClientTest extends TestCase
{
 ...
 public function testRetencionObtieneClienteDeLaUbicacionDelPase():void{$repo=file_get_contents(dirname(__DIR__,2).'/app/Repositories/MaintenanceRepository.php');self::assertStringContainsString('JOIN locations l ON l.id=vp.location_id',$repo);self::assertStringContainsString("SELECT l.client_id,'visitor_identification'",$repo);self::assertStringNotContainsString("SELECT vp.client_id,'visitor_identification'",$repo);}
 public function testLimpiezaTemporalConservaGitkeep():void{$service=file_get_contents(dirname(__DIR__,2).'/app/Services/CronService.php');self::assertStringContainsString("getFilename()==='.gitkeep'",$service);}
}
