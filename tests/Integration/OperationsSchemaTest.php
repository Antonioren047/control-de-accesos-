...
use PDO;use PHPUnit\Framework\TestCase;use Vigilancia\Database\Connection;use Vigilancia\Repositories\OperationalRepository;use Vigilancia\Support\Config;
final class OperationsSchemaTest extends TestCase
{
 ...
 public function testReferenciaManualResuelveUnaCredencialActiva():void{$pdo=$this->connection();$reference=$pdo->query("SELECT token_reference FROM guard_credentials WHERE status='active' ORDER BY id DESC LIMIT 1")->fetchColumn();if(!$reference)self::markTestSkipped('No hay credenciales activas.');$credential=(new OperationalRepository($pdo))->credentialByReference((string)$reference);self::assertNotNull($credential);self::assertGreaterThan(0,(int)$credential['guard_user_id']);}
}
