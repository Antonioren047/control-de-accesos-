...
return static function(PDO$pdo,bool$demo=false):void{
    ...
    $s=$pdo->prepare("INSERT INTO events(event_type_id,operational_session_id,client_id,location_id,access_point_id,guard_user_id,title,description,priority,status,occurred_at,created_by,created_at,updated_at) VALUES(?,?,?,?,?,?,'Incidencia demostrativa','Incidencia demostrativa de la Fase 8','important','submitted',UTC_TIMESTAMP(),?,UTC_TIMESTAMP(),UTC_TIMESTAMP())");$s->execute([$row['type_id'],$row['session_id'],$row['client_id'],$row['location_id'],$row['access_point_id'],$row['guard_user_id'],$row['guard_user_id']]);
};
